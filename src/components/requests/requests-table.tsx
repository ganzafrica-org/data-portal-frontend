...
export default function RequestsTable({
  ...
  const visibleRequests = DUMMY_REQUESTS.filter((req) =>
    canViewRequest(req.userId),
  );
  ...
  const getPriorityLevel = (request: DataRequest) => {
    ...
    if (requester?.userType === "government_agency")
      return { level: "High", color: "bg-red-100 text-red-800" };
    if (requester?.userType === "academic_institution")
  ...
  const filteredRequests = visibleRequests.filter((request) => {
    ...
    const matchesUserType =
      userTypeFilter === "all" || requester?.userType === userTypeFilter;

    const priority = getPriorityLevel(request);
    const matchesPriority =
      priorityFilter === "all" ||
      priority.level.toLowerCase() === priorityFilter;
  ...
  const userTypes = Array.from(
    new Set(
      visibleRequests
        .map((req) => getUserInfo(req.userId)?.userType)
        .filter(Boolean),
    ),
  );
                                      ...
                                      <div className="flex flex-wrap gap-1 mt-1">
                                        {request.supportingDocuments
                                          ...
                                          .map((doc, idx) => (
                                            <Badge
                                              key={idx}
                                              className={getDocumentCategoryColor(
                                                doc.category,
                                              )}
                                              variant="outline"
                                            >
